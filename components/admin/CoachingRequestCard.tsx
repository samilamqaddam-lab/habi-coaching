...
interface CoachingRequestCardProps {
...
export default function CoachingRequestCard({
  request,
  onStatusChange,
  onContactedChange,
  onNotesChange,
      ...
      >
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="text-right flex-shrink-0">
            <div className="inline-block px-3 py-1.5 bg-purple-400/10 text-purple-400 rounded-lg text-sm font-medium border border-purple-400/20">
              {request.coaching_packages?.name || 'Package inconnu'}
            </div>
            <div className="text-sm text-slate-500 mt-1">
              {request.coaching_packages?.price} DH
            </div>
          </div>
      ...
      {isExpanded && (
        <div className="border-t border-slate-700 p-5 space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <h4 className="text-xs font-medium text-slate-500 uppercase tracking-wide mb-2">Package</h4>
              <div className="space-y-1">
                <p className="text-slate-300 font-medium">{request.coaching_packages?.name}</p>
                <p className="text-sm text-slate-400">
                  {request.coaching_packages?.session_count} séance(s) × {request.coaching_packages?.session_duration} min
                </p>
                <p className="text-sm text-slate-400">
                  Format préféré: <span className="text-slate-300">{formatLabel[request.preferred_format]}</span>
                </p>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full bg-slate-900/50 border border-slate-700 rounded-lg p-3 text-slate-300 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-purple-400/50 focus:border-purple-400"
              rows={3}
            />
            {notes !== (request.notes || '') && (
              <button
                ...
                className="mt-2 px-4 py-1.5 bg-purple-400/20 text-purple-400 rounded-lg text-sm font-medium hover:bg-purple-400/30 transition-colors disabled:opacity-50"
              >
          ...
          <div className="flex flex-wrap items-center gap-3 pt-3 border-t border-slate-700">
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="bg-slate-900 border border-slate-700 rounded-lg px-3 py-1.5 text-sm text-slate-300 focus:outline-none focus:ring-2 focus:ring-purple-400/50"
              >
            ...
            {request.status !== 'cancelled' && (
              <button
                onClick={() => onSendPaymentRequest(request.id)}
                className="flex items-center gap-2 px-4 py-1.5 bg-green-400/20 text-green-400 rounded-lg text-sm font-medium hover:bg-green-400/30 transition-colors"
              >
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
