...
export default function RessourcesContent({ articles }: RessourcesContentProps) {
  const { t, locale } = useTranslation()

  // Fallback articles from translations if no Sanity articles
  ...
  const categoryLabels: Record<string, string> = {
    wellness: locale === 'fr' ? 'Bien-être' : 'Wellness',
    yoga: 'Yoga',
    coaching: 'Coaching',
    leadership: 'Leadership',
    'personal-growth': locale === 'fr' ? 'Développement personnel' : 'Personal Growth',
  }

  // Use Sanity articles if available, otherwise fallback
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {displayArticles
            ? displayArticles.map((article) => (
                <Card key={article._id} hover padding="lg" className="flex flex-col">
                  <div className="aspect-[16/9] bg-gradient-to-br from-terracotta/10 to-sage/10 rounded-xl mb-6 flex items-center justify-center overflow-hidden">
                  ...
                  <Button variant="text" href={`/blog/${article.slug.current}`}>
